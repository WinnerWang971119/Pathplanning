...
**The `_predict.py` substrate.** `planners/_predict.py` is pure (plain floats in, deterministic output, no irsim, no RNG). It exports:
...
**`d_star_lite_predictive` (planned, not yet registered).** The lidar-only variant (`LidarTracker`, `geometry = "cone"`) is planned but blocked behind the oracle go/no-go checkpoint (T10). It is NOT registered in `ALGORITHMS` yet. Document it as the planned follow-on; do not invoke it as if it exists.
...
**Horizon sweep tooling.** `runners/run_horizon_sweep.py` shells `run_experiment` once per horizon in `{0, 5, 10, 20}` for `d_star_lite_oracle`, writing label dirs `d_star_lite_oracle_h<steps>/`. `runners/plot_horizon_sweep.py` (headless, read-only) reads those dirs and writes `failure_rate_vs_horizon.png`, `median_time_vs_horizon.png`, and `horizon_sweep_summary.csv` to `results/<world_stem>/horizon_sweep_plots/` (x-axis = T seconds = steps × 0.1). `python -m runners.plot_horizon_sweep --selfcheck` runs synthetic-fixture cases (no irsim) and exits 0 only if all pass.
...
**TC53–TC62** (added to `python arena/arena.py arena/arena_v1.yaml --check`, growing it from 55 to 66 cases):
