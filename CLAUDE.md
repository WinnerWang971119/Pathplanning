...
**`EXPERIMENTAL_KEYS` and the canonical-set carve-out.** `planners/_grid.py` defines `PREDICT_FAMILIES = frozenset({"d_star_lite_oracle", "d_star_lite_predictive"})` and `EXPERIMENTAL_KEYS = frozenset({"d_star_lite_oracle"})` — a STRICT SUBSET of `PREDICT_FAMILIES` (membership here is only the canonical-set carve-out, NOT the horizon/label machinery). `run_all.py`'s import-time assertion holds as `set(_CANONICAL_ORDER) == set(ALGORITHMS) - EXPERIMENTAL_KEYS`, so only the oracle is registered/usable through the runner/sweep tooling without landing on the canonical scatter; `d_star_lite_predictive` is a canonical member of `_CANONICAL_ORDER` (run at `PREDICT_HORIZON=10`).
