...
```powershell
.venv\Scripts\Activate.ps1
python arena/arena.py arena/arena_v1.yaml --check     # 47 PASS = harness healthy (TC1-TC46; estimate ~50 min, dominated by the full-episode traffic-on and --no-traffic solve subprocess TCs)
...
**TC38–TC46** (added to `python arena/arena.py arena/arena_v1.yaml --check`):
