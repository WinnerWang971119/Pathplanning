...
**The criterion.** A canonical-50 seed is `degenerate` iff, for **all required planners** (default all 13 = the 11 canonical + `d_star_lite_oracle_h10` + `d_star_lite_predictive_h10`), the episode is a crash whose first crashed trace line falls at `1 <= step <= window_steps` (`window_steps = round(window_seconds / step_time)`, default `4.0 / 0.1 = 40`; step 0 is the runner's hardcoded `crashed=False` sentinel, so the earliest observable crash is step 1). Missing data for any required planner (no metrics file, and no DNF roster entry) makes the seed `indeterminate` — never dropped. A single survivor (goal reached, timed out, a crash after the window, `planner_error`, or a wallclock-killed DNF — none of which is a confirmed instant crash) makes the seed `kept`. `--window-seconds` / `--step-time` override the window; the resolved values are recorded in the sidecar.
...
**The all-13 no-op trap.** On the standard `run_all -> plot` tree only the 11 canonical dirs exist (`run_all` never produces the two predictive keys), so the default `--planners all13` returns `indeterminate` (exit 3), naming the missing dirs in the printed verdict and the sidecar's `missing_labels` — a loud no-op, never a silent one. Use `--planners canonical` to filter on the 11 deliberately, or run the two `d_star_lite_oracle`/`d_star_lite_predictive` experiments at `--predict-horizon 10` first.
