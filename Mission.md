...
- matplotlib, static PNG, generated by `runners/plot.py` reading
  `results/<world_stem>/<label>/*.json`.

_Landed: `runners/plot.py` (read-only) renders the headline A1 scatter plus six
supporting charts (A3 / A4 / B1 / B2 / B3 / B4) and a `summary.csv` into
`results/<world_stem>/plots/`; `runners/run_all.py` drives all 11 canonical
planners to produce its input. The candidate A2 Pareto-frontier overlay was
deselected; the replan families are plotted at the fixed K=5 the rest of the
study uses; failure rate is extended past crash+timeout to also count
planner_error and DNF (wallclock-killed) episodes. The Phase 6b K-sweep remains
deferred. See "The Phase 5 plotter and batch driver" in CLAUDE.md._

---

## Phase 6 — Algorithms

_Mostly landed: the unified `Controller` interface (`reset` + `act`), the
planner-agnostic runner loop, the grid family (`a_star_once`, `a_star_replan`,
`dijkstra_once`, `dijkstra_replan`), `d_star_lite`, the reactive family (`dwa`,
`apf`), and the sampling family (`rrt_once`, `rrt_replan`, `rrt_star_once`,
`rrt_star_replan`) all ship; all 11 planner keys land. Only the Phase 6b K-sweep
remains deferred. See "The planner family (Phase 6)" in CLAUDE.md._
